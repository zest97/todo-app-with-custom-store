...
const createStore = function (reducer) {
  let state;
  let listeners = []; // getting the state

  const getState = () => state; // listener
...
function addTodoOnClick() {
  ...
  input.value = '';
  appStates.dispatch(addTodoActionCreator({
    id: generateId(),
    name,
    complete: false
  }));
...
function addGoalOnClick() {
  ...
  const name = input.value;
  input.value = '';
  appStates.dispatch(addGoalActionCreator({
    id: generateId(),
    name
  }));
}
...
function addTodoToDom(todo) {
  ...
  node.addEventListener("click", () => {
    appStates.dispatch(toggleTodoActionCreator(todo.id));
  });
  const removeBtn = createRemoveButton(() => {
    appStates.dispatch(removeTodoActionCreator(todo.id));
  });
...
function addGoalToDom(goal) {
  ...
  const removeBtn = createRemoveButton(() => {
    appStates.dispatch(removeGoalActionCreator(goal.id));
  });
  node.appendChild(text);
  node.appendChild(removeBtn);
